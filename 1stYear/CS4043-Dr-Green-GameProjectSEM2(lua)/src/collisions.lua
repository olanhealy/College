function checkCollisions()
    ...
    for a = 1, blocks.numChildren, 1 do
        if (collisionRect.y - 10 > blocks[a].y - 170 and blocks[a].x - 40 < collisionRect.x and blocks[a].x + 10 > collisionRect.x) then
            speed = 0
            killHero()
        end
    end

    if (math.abs(hero.x - enemy.x) <= 10 and collisionRect.y + 3 >= enemy.x - 3) then
        speed = 0
        killHero()
    end

    if (math.abs(hero.x - potion.x) <= 10 and collisionRect.y <= potion.y) then
        potionActive = true
        hero.gravity = -3
